...
function setup() {
	...
	lastPlayerMove = new lineObj(0, 0, color(0, 0, 0, 0), 0, boxSize, dotSize);
...
function startGame() {
	...
	undoButton.position(width - 100, height - 25);
	undoButton.mousePressed(undoMove);
}

function undoMove() {
		
	var moveToUndo = playerMovesList.pop();
	
	// remove last line
	for (var i = 0; i < (boardSize * 2); i++) {
		for (var j = 0; j < boardSize; j++) {
			if( (lineArr[i][j].centerX == moveToUndo.centerX) && (lineArr[i][j].centerY == moveToUndo.centerY) ) {
				lineArr[i][j].isActive = false;
				lineArr[i][j].lineColor = color(0, 0, 0, 0);
			}
		}
	}
			
	// update box totals and score
	console.log(totalFilledBoxes);
	var numBoxesMade = checkGameBoard();
	console.log(numBoxesMade);
	var undoneBoxes = totalFilledBoxes - numBoxesMade[1];
	// check to see if a box was unmade
	if (numBoxesMade[1] < totalFilledBoxes) {
		if (turn) {
			playerOne.playerScore -= undoneBoxes;
		} else {
			playerTwo.playerScore -= undoneBoxes;
		}
		totalFilledBoxes = numBoxesMade[1];
	} else {
		// change back turn
		turn = !turn;
	}
...
function draw() {
	...
	if (setPlayerColors) {
		...
		//text(currTurnTime, 15, height-15);
		push();
		rectMode(CORNER);
		fill(0,255,0);
		var tempTime = map(currTurnTime,0,turnTime,0,100);
		rect(15,height-15,tempTime,15);
		pop();
		displayBoard();
		...
		if (computerPlayer && !turn) {
			...
			if (numBoxesMade[0] > 0) {
				playerTwo.playerScore += numBoxesMade[0];
				totalFilledBoxes += numBoxesMade[0];
			} else {
				turn = !turn;
			}
			computerChoice = new lineObj(0, 0, color(0, 0, 0, 0), 0, boxSize, dotSize);
		...
		if (playerChoice.isActive) {
			...
			if (numBoxesMade[0] > 0) {
				if (turn) {
					playerOne.playerScore += numBoxesMade[0];
				} else {
					playerTwo.playerScore += numBoxesMade[0];
				}
				totalFilledBoxes += numBoxesMade[0];
			...
			playerMovesList.push(playerChoice);
			playerChoice = new lineObj(0, 0, color(0, 0, 0, 0), 0, boxSize, dotSize);
			currTurnTime = turnTime;
		}
	...
	if(computerPlayer && !gameOver) {
		...
		rectMode(CENTER);
		rect(lastComputerMove.centerX, lastComputerMove.centerY, lastComputerMove.width, lastComputerMove.height);
		pop();
	}
	if(lastPlayerMove.isActive && !gameOver) {
		push();
		noStroke();
		fill(color(255,255,255,150));
		rectMode(CENTER);
		rect(lastPlayerMove.centerX, lastPlayerMove.centerY, lastPlayerMove.width, lastPlayerMove.height);
		pop();
	}
	
	// auto play if time expires
	if(currTurnTime <= 0) { 
	
		makeComputerChoice();
			lastPlayerMove = computerChoice;
			// find the choice in the line Array and update the line array
			for (var i = 0; i < (boardSize * 2); i++) {
				for (var j = 0; j < boardSize; j++) {
					if( (lineArr[i][j].centerX == lastPlayerMove.centerX) && (lineArr[i][j].centerY == lastPlayerMove.centerY) ) {
						//console.log("MATCHING LINE AND COMPUTER CHOICE FOUND!");
						//console.log(lineArr[i][j]);
						lineArr[i][j].isActive = true;
						lineArr[i][j].lineColor = getTurnColor();
		...
		for (var j = 0; j < boardSize - 1; j++) {
			...
			if (!boxArr[i][j].isLocked) {
				...
				if (lineArr[topLineX][j].isActive && lineArr[rightLineX][j].isActive && lineArr[leftLineX][j].isActive && lineArr[bottomLineX][nextj].isActive) {
					...
					boxArr[i][j].isLocked = false;
				}
			}
		}
	}
	
	returnArr.push(boxesMade);
	returnArr.push(totalBoxes);
	return returnArr;
}
...
function makeComputerChoice() {
	...
	// pick the first valid play it finds

	// computer Difficulty 2
	// prioritize any line that makes a box, then random
...
class boxObj {
	...
	display() {
		// draw 4 dots and 4 lines
		// if box is locked, fill center
		noStroke();
		rectMode(CENTER);
		if(this.isLocked) {
			fill(this.boxColor);
			rect(this.centerX,this.centerY,boxSize-dotSize-2,boxSize-dotSize-2);
		}
		if(this.topLine.isActive) {
			fill(this.topLine.lineColor);
			rect(this.topLine.centerX,this.topLine.centerY,this.topLine.width-2,this.topLine.height-2);
		}
		if(this.rightLine.isActive) {
			fill(this.rightLine.lineColor);
			rect(this.rightLine.centerX,this.rightLine.centerY,this.rightLine.width-2,this.rightLine.height-2);
		}
		if(this.leftLine.isActive) {
			fill(this.leftLine.lineColor);
			rect(this.leftLine.centerX,this.leftLine.centerY,this.leftLine.width-2,this.leftLine.height-2);
		}
		if(this.bottomLine.isActive) {
			fill(this.bottomLine.lineColor);
			rect(this.bottomLine.centerX,this.bottomLine.centerY,this.bottomLine.width-2,this.bottomLine.height-2);
		}
		
		fill(255);
		ellipse(this.centerX-boxSize/2,this.centerY-boxSize/2,dotSize,dotSize);
		ellipse(this.centerX-boxSize/2,this.centerY+boxSize/2,dotSize,dotSize);
		ellipse(this.centerX+boxSize/2,this.centerY-boxSize/2,dotSize,dotSize);
		ellipse(this.centerX+boxSize/2,this.centerY+boxSize/2,dotSize,dotSize);
	}
